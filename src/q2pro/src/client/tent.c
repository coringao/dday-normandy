...
static explosion_t *CL_PlainExplosion(void)
{
    ...
    VectorCopy(tent.pos1, ex->ent.origin);
    ex->type = ex_poly;
...
static void CL_ParseLaser(int colors)
{
    ...
    VectorCopy(tent.pos1, l->start);
    VectorCopy(tent.pos2, l->end);
    l->lifetime = 100;
...
// override any beam with the same source AND destination entities
    for (i = 0, b = cl_beams; i < MAX_BEAMS; i++, b++)
        if (b->entity == tent.entity1 && b->dest_entity == tent.entity2)
            goto override;
...
override:
            b->entity = tent.entity1;
            b->dest_entity = tent.entity2;
            b->model = model;
            b->endtime = cl.time + 200;
            VectorCopy(tent.pos1, b->start);
            VectorCopy(tent.pos2, b->end);
            VectorCopy(tent.offset, b->offset);
            return;
        }
    }
}
...
static void CL_ParseSteam(void)
{
    ...
    if (tent.entity1 == -1) {
        CL_ParticleSteamEffect(tent.pos1, tent.dir, tent.color & 0xff, tent.count, tent.entity2);
        return;
    }
    ...
    s->id = tent.entity1;
    s->count = tent.count;
    VectorCopy(tent.pos1, s->org);
    VectorCopy(tent.dir, s->dir);
    s->color = tent.color & 0xff;
    s->magnitude = tent.entity2;
    s->endtime = cl.time + tent.time;
...
static void CL_RailSpiral(void)
{
    ...
    VectorCopy(tent.pos1, move);
    VectorSubtract(tent.pos2, tent.pos1, vec);
...
{
    angles[0] = acos(tent.dir[2]) / M_PI * 180;
    if (tent.dir[0])
        angles[1] = atan2(tent.dir[1], tent.dir[0]) / M_PI * 180;
    else if (tent.dir[1] > 0)
        angles[1] = 90;
    else if (tent.dir[1] < 0)
        angles[1] = 270;
    else
...
void CL_ParseTEnt(void)
{
    ...
    case TE_BLOOD:          // bullet hitting flesh
        if (!(cl_disable_particles->integer & NOPART_BLOOD))
            CL_ParticleEffect(tent.pos1, tent.dir, 0xe8, 60);
        break;

    case TE_GUNSHOT:            // bullet hitting wall
    case TE_SPARKS:
    case TE_BULLET_SPARKS:
        if (tent.type == TE_GUNSHOT)
            CL_ParticleEffect(tent.pos1, tent.dir, 0, 40);
        else
            CL_ParticleEffect(tent.pos1, tent.dir, 0xe0, 6);

        if (tent.type != TE_SPARKS) {
            CL_SmokeAndFlash(tent.pos1);

            // impact sound
            r = rand() & 15;
            if (r == 1)
                S_StartSound(tent.pos1, 0, 0, cl_sfx_ric1, 1, ATTN_NORM, 0);
            else if (r == 2)
                S_StartSound(tent.pos1, 0, 0, cl_sfx_ric2, 1, ATTN_NORM, 0);
            else if (r == 3)
                S_StartSound(tent.pos1, 0, 0, cl_sfx_ric3, 1, ATTN_NORM, 0);
        }
        break;

    case TE_SCREEN_SPARKS:
    case TE_SHIELD_SPARKS:
        if (tent.type == TE_SCREEN_SPARKS)
            CL_ParticleEffect(tent.pos1, tent.dir, 0xd0, 40);
        else
            CL_ParticleEffect(tent.pos1, tent.dir, 0xb0, 40);
        //FIXME : replace or remove this sound
        S_StartSound(tent.pos1, 0, 0, cl_sfx_lashit, 1, ATTN_NORM, 0);
        break;

    case TE_SHOTGUN:            // bullet hitting wall
        CL_ParticleEffect(tent.pos1, tent.dir, 0, 20);
        CL_SmokeAndFlash(tent.pos1);
        break;

    case TE_SPLASH:         // bullet hitting water
        if (tent.color < 0 || tent.color > 6)
            r = 0x00;
        else
            r = splash_color[tent.color];
        CL_ParticleEffect(tent.pos1, tent.dir, r, tent.count);

        if (tent.color == SPLASH_SPARKS) {
            r = rand() & 3;
            if (r == 0)
                S_StartSound(tent.pos1, 0, 0, cl_sfx_spark5, 1, ATTN_STATIC, 0);
            else if (r == 1)
                S_StartSound(tent.pos1, 0, 0, cl_sfx_spark6, 1, ATTN_STATIC, 0);
            else
                S_StartSound(tent.pos1, 0, 0, cl_sfx_spark7, 1, ATTN_STATIC, 0);
        }
        break;

    case TE_LASER_SPARKS:
        CL_ParticleEffect2(tent.pos1, tent.dir, tent.color, tent.count);
        break;

    case TE_BLUEHYPERBLASTER:
        CL_BlasterParticles(tent.pos1, tent.dir);
        break;
    ...
    case TE_FLECHETTE:          // flechette
        ...
        case TE_BLASTER:
            ...
            break;
        case TE_BLASTER2:
            CL_BlasterParticles2(tent.pos1, tent.dir, 0xd0);
            ex->ent.skinnum = 1;
            ex->lightcolor[1] = 1;
            break;
        case TE_FLECHETTE:
            CL_BlasterParticles2(tent.pos1, tent.dir, 0x6f);  // 75
            ex->ent.skinnum = 2;
    ...
    case TE_RAILTRAIL:          // railgun effect
        CL_RailTrail();
        S_StartSound(tent.pos2, 0, 0, cl_sfx_railg, 1, ATTN_NORM, 0);
        break;
    ...
    case TE_GRENADE_EXPLOSION_WATER:
        ...
        if (!(cl_disable_particles->integer & NOPART_GRENADE_EXPLOSION))
            CL_ExplosionParticles(tent.pos1);

        if (tent.type == TE_GRENADE_EXPLOSION_WATER)
            S_StartSound(tent.pos1, 0, 0, cl_sfx_watrexp, 1, ATTN_NORM, 0);
        else
            S_StartSound(tent.pos1, 0, 0, cl_sfx_grenexp, 1, ATTN_NORM, 0);
        break;
    ...
    case TE_PLASMA_EXPLOSION:
        CL_PlainExplosion();
        CL_ExplosionParticles(tent.pos1);
        S_StartSound(tent.pos1, 0, 0, cl_sfx_rockexp, 1, ATTN_NORM, 0);
        break;
    ...
    case TE_ROCKET_EXPLOSION_WATER:
        ...
        if (tent.type == TE_ROCKET_EXPLOSION_WATER)
            S_StartSound(tent.pos1, 0, 0, cl_sfx_watrexp, 1, ATTN_NORM, 0);
        else
            S_StartSound(tent.pos1, 0, 0, cl_sfx_rockexp, 1, ATTN_NORM, 0);
        break;
    ...
    case TE_BFG_BIGEXPLOSION:
        CL_BFGExplosionParticles(tent.pos1);
        break;
    ...
    case TE_BUBBLETRAIL:
        CL_BubbleTrail(tent.pos1, tent.pos2);
        break;

    case TE_PARASITE_ATTACK:
    case TE_MEDIC_CABLE_ATTACK:
        VectorClear(tent.offset);
        tent.entity2 = 0;
        CL_ParseBeam(cl_mod_parasite_segment);
        break;

    case TE_BOSSTPORT:          // boss teleporting to station
        CL_BigTeleportParticles(tent.pos1);
        S_StartSound(tent.pos1, 0, 0, S_RegisterSound("misc/bigtele.wav"), 1, ATTN_NONE, 0);
        break;
    ...
    case TE_GREENBLOOD:
        CL_ParticleEffect2(tent.pos1, tent.dir, 0xdf, 30);
        break;

    case TE_TUNNEL_SPARKS:
        CL_ParticleEffect3(tent.pos1, tent.dir, tent.color, tent.count);
        break;

    case TE_LIGHTNING:
        S_StartSound(NULL, tent.entity1, CHAN_WEAPON, cl_sfx_lightning, 1, ATTN_NORM, 0);
        VectorClear(tent.offset);
        CL_ParseBeam(cl_mod_lightning);
        break;

    case TE_DEBUGTRAIL:
        CL_DebugTrail(tent.pos1, tent.pos2);
        break;
    ...
    case TE_FLASHLIGHT:
#if USE_DLIGHTS
        CL_Flashlight(tent.entity1, tent.pos1);
#endif
        break;

    case TE_FORCEWALL:
        CL_ForceWall(tent.pos1, tent.pos2, tent.color);
        break;

    case TE_HEATBEAM:
        VectorSet(tent.offset, 2, 7, -3);
    ...
    case TE_HEATBEAM_SPARKS:
        CL_ParticleSteamEffect(tent.pos1, tent.dir, 0x8, 50, 60);
        S_StartSound(tent.pos1,  0, 0, cl_sfx_lashit, 1, ATTN_NORM, 0);
        break;

    case TE_HEATBEAM_STEAM:
        CL_ParticleSteamEffect(tent.pos1, tent.dir, 0xE0, 20, 60);
        S_StartSound(tent.pos1,  0, 0, cl_sfx_lashit, 1, ATTN_NORM, 0);
        break;
    ...
    case TE_BUBBLETRAIL2:
        CL_BubbleTrail2(tent.pos1, tent.pos2, 8);
        S_StartSound(tent.pos1,  0, 0, cl_sfx_lashit, 1, ATTN_NORM, 0);
        break;

    case TE_MOREBLOOD:
        CL_ParticleEffect(tent.pos1, tent.dir, 0xe8, 250);
        break;

    case TE_CHAINFIST_SMOKE:
        VectorSet(tent.dir, 0, 0, 1);
        CL_ParticleSmokeEffect(tent.pos1, tent.dir, 0, 20, 20);
        break;

    case TE_ELECTRIC_SPARKS:
        CL_ParticleEffect(tent.pos1, tent.dir, 0x75, 40);
        //FIXME : replace or remove this sound
        S_StartSound(tent.pos1, 0, 0, cl_sfx_lashit, 1, ATTN_NORM, 0);
        break;
...
#if USE_DLIGHTS
        CL_ColorFlash(tent.pos1, 0, 150, -1, -1, -1);
#endif
        CL_ColorExplosionParticles(tent.pos1, 0, 1);
        S_StartSound(tent.pos1, 0, 0, cl_sfx_disrexp, 1, ATTN_NORM, 0);
        break;

    case TE_TELEPORT_EFFECT:
    case TE_DBALL_GOAL:
        CL_TeleportParticles(tent.pos1);
        break;
