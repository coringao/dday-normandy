...
void InitGame (void)
{
	...
	STARTPERFORMANCE(1);
	STARTPERFORMANCE(2);
	dllglobals->Init(); //be careful with all functions called from this one (like dprintf_internal) to not use proxyinfo pointer because it's not initialized yet. -Harven
					...
					while (1)
						{
							...
							q2a_memcpy (keyname, com_tok, sizeof(keyname)-1);
							...
							if(!Q_stricmp("classname", keyname) && checkDisabledEntities(com_tok))
								{
									classnamepos[0] = '_';  // change the 'classname' entry to '_lassname', this makes the q2 code ignore it.
...
qboolean ClientConnect (edict_t *ent, char *userinfo)
{
	...
	// always clearout just in case there isn't any clients (therefore runframe doesn't get called)
	if(maxReconnectList)
		{
			unsigned int i;
...
//*** UPDATE END ***
	...
	q2a_memcpy (proxyinfo[client].name, s, sizeof(proxyinfo[client].name)-1);
	...
	q2a_memcpy (proxyinfo[client].skin, skinname, sizeof(proxyinfo[client].skin)-1);
	
	//   q2a_strcpy(ent->client->pers.netname, proxyinfo[client].name);
	q2a_memcpy (proxyinfo[client].userinfo, userinfo, sizeof(proxyinfo[client].userinfo) - 1);
	...
	else if(checkClientIpAddress && proxyinfo[client].ipaddress[0] == 0) // check for invalid IP's and don't let them in :)
		{
...
qboolean checkForNameChange(int client, edict_t *ent, char *userinfo)
{
	...
	q2a_memcpy (newname, s, sizeof(newname) - 1);
...
qboolean checkForSkinChange(int client, edict_t *ent, char *userinfo)
{
	...
	q2a_memcpy (newskin, s, sizeof(newskin) - 1);
	...
	if(cl_pitchspeed_enable)
		{
			...
			if(newps == 0)
			...
			else
				{
					...
					// ignore changes to 150 because action quake2 generates them itself...
	...
	if(cl_anglespeedkey_enable)
		{
			...
			if(newas == 0.0)
			...
			else
				{
					...
					// ignore changes to 1.5 because action quake2 generates them itself...
