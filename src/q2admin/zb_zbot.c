...
//*** UPDATE START ***
				...
				else if(command == QCMD_PRIVATECOMMAND)
				{
					...
					{
						//check each command, if we didn't get a response log it
				...
				else if(command == QCMD_SPAMBYPASS)
				{
					if(proxyinfo[client].q2a_bypass)
					{
						gi.bprintf(PRINT_HIGH, "\n %s has logged on without an anti-cheat client because of an arrangement\n with the server admin.  This is most likely because %s is using a linux\n or mac client - contact the server admin if you have issues with %s.\n", proxyinfo[client].name, proxyinfo[client].name, proxyinfo[client].name);
					}
				}
				...
				else if(command == QCMD_TESTCMDQUEUE)
				{
					...
					{
						//all 3 checks came through fine
