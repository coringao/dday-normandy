...
void explosion_effect (vec3_t start)
{
	edict_t *explo;
...
//		gi.dprintf ("%s  %s\n", vtos(self->owner->velocity),vtos(self->owner->client->oldvelocity));

		//keeps player from getting stuck in weird spots when starting
		if (self->owner->client->turret_time > level.time -.8)
			self->owner->movetype = MOVETYPE_NOCLIP;
		else if (self->obj_gain != 1) //(not a tank)
			self->owner->movetype = MOVETYPE_FLYMISSILE; 
...
//			gi.bprintf (PRINT_HIGH, "%s   %s\n", t->target, vtos(t->s.origin));
	...
	self->flags |= FL_TEAMSLAVE;
	self->client->turret_time = level.time;
}
...
qboolean CheckForTurret(edict_t *ent)
{
	...
	if (ent->client->turret_time != level.time)
		if (ent->client->turret_time > level.time - 1)
			return false;
