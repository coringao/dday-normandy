...
void rotating_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	if (self->deathtarget)
	{
...
//	ent->moveinfo.sound_middle = "doors/hydro1.wav";
	...
	if (ent->health)
	{
		...
		if (ent->obj_gain)
			ent->die =	func_explosive_objective_explode;
		else
			ent->die = rotating_die;
	}
...
void door_go_down (edict_t *self)
{
	...
	self->moveinfo.state = STATE_DOWN;
	if (self->classnameb == FUNC_DOOR)
		Move_Calc (self, self->moveinfo.start_origin, door_hit_bottom);
	else if (self->classnameb == FUNC_DOOR_ROTATING)
...
void Think_SpawnDoorTrigger (edict_t *ent)
{
	...
	maxs[0] += 60;
	maxs[1] += 60;
	mins[2] -=5;//faf: so ceiling doors can be triggered from underneath with new hitboxes
...
void door_blocked  (edict_t *self, edict_t *other)
{
	...
	//keep bots from blocking doors
	if (other->client && other->ai)
	{
		other->solid = SOLID_NOT;
		other->ai->door_block_time = level.time;
		return;
	}
...
void door_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (self->classnameb == FUNC_DOOR_ROTATING)
	{
		if (other->ai)
			other->ai->door_touch_time = level.time;
	}
...
{
	ent->classnameb = FUNC_DOOR_ROTATING;
	VectorClear (ent->s.angles);
...
void SP_func_train (edict_t *self)
{
	...
	if (!strcmp(level.mapname,"resist"))
		self->dmg = 0;
...
void trigger_elevator_init (edict_t *self)
{
	...
	if (self->movetarget && self->movetarget->classnameb != FUNC_TRAIN)
	{
		...
		if (other->client->resp.team_on->index != ent->obj_owner)
		{
				...
				other->client->enter_spawn_time < level.time - 4)
			{
				if (ent->message)
					...
					safe_centerprintf(other, "You've Entered Spawn Area, Stay Out!!\n");
